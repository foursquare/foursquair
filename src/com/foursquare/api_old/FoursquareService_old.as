package com.foursquare.api
{
	...
	public class FoursquareService_old extends Actor implements IFoursquareService
	{
		...
		public function FoursquareService_old()
		{
		...
		public function checkin(shout : String="", venueVO:VenueVO=null) : void
		{
			...
			var request : URLRequest = oauth.buildRequest(	URLRequestMethod.POST, url, new OAuthToken(model.oauth_token, model.oauth_token_secret), params);
		...
		private function getData(url : String, onSuccess : Function, useGET : Boolean=false, requestParams : Object=null) : void
		{
			var request : URLRequest;
			if (!useGET)
			{
				request = oauth.buildRequest(	URLRequestMethod.POST, url, 
		...
		private function blah():void{
			var loader : URLLoader = new URLLoader();
			loader.addEventListener(IOErrorEvent.IO_ERROR, onIEError);
			loader.addEventListener(Event.COMPLETE, onBlahComplete);
			loader.load(request);
		}
		
		private function onBlahComplete(event:Event):void{
				var loader : URLLoader = e.target as URLLoader;
				try
				{
		...
		private function onIOError(event:IOErrorEvent):void{
			var error : Error = new Error("IOError: couldnt talk to foursquare.", event.errorID);
			handleError(error);
		}
